...
struct sounder
{
	...
	void initsounds()
	{
		...
		soundeffect = (Mix_LoadWAV("slapsound.wav"));
		moreSounds = (Mix_LoadWAV("enemmyslaps.wav"));
		pop = (Mix_LoadWAV("slapsound3.wav"));
		music = (Mix_LoadMUS("d_e1m1.mid"));
		tim = Mix_LoadWAV("tim.wav");
		Mix_VolumeChunk(soundeffect,30);
		Mix_VolumeChunk(tim, 30);
		Mix_VolumeChunk(pop, 30);
		Mix_VolumeChunk(moreSounds, 30);
		Mix_VolumeMusic(30);
	}
...
struct gameObj
{
	...
	bool collision(gameObj* obj, float elapsed)
	{
		...
		if (inrange)
		{
			Mix_PlayChannel(-1, worldsounds->pop, 0);
		}
...
struct alien : gameObj
{
	...
	bool getHit(gameObj *bang, float elapsed)
	{
		...
		{
			Mix_PlayChannel(-1, worldsounds->tim, 0);
	...
	void checkHits(gameObj* well, float elapsed)
	{
		for (int i = 0; i < 5; i++)
		{
			for (alien* n : armyItself[i])
			{
				...
				{
					well->alive = false;
					Mix_PlayChannel(-1, worldsounds->moreSounds, 0);
					return;
				}
			}
		}
...
struct player : gameObj
{
	...
	void move(float elapsed, float dir)
	{
		Mix_PlayChannel(-1, worldsounds->soundeffect, 0);
		float tmpx = x + dir * speed * elapsed;
		if (tmpx < 0)
		{
			if (fabs(tmpx) <= maxx)
...
int main(int argc, char *argv[])
{
	...
	Mix_PlayMusic(worldsounds->music, -1);
	if (player == NULL)
	{
		//("IMAGEERROR");
	}
